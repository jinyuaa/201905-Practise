# 在 ID3 的基础上进一步计算增益率 作为最优划分属性为 C45
# 增益率(Gain_ratio(D,a)) = 信息增益(Gain(D,a)) / 固有值(IV(a))
# 固有值为0情况 如何处理
...
def choos_feature(dataset):
    ...
    for i in range(number_label):
        ...
        for value in uniqu_vals:
            ...
            new_iv += -prob * log(prob, 2)
        print("第%d特征的固有属性为：%.3f" % (i, new_iv))
        # 计算属性的信息增益
        info_gain = root_comentropy - new_comentropy
        # 打印每个特征的信息增益
        # print("第%d个特征的信息增益为%.3f" % (i, info_gain))
        # ----计算增益率----
        info_gain_ratio = info_gain / new_iv
        print("第%d个特征的增益率为%.3f" % (i, info_gain_ratio))
